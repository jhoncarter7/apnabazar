  ...
  return (
    <div className="p-6 ">
      ...
      <div className="grid  grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 overflow-hidden gap-y-4 ">
        {productsData.map((product)=>  <ProductsCard key={product._id} title={product.title} oldPrice={product.oldPrice} newPrice={product.newPrice} description={product.description} imageUrl={product.imageUrl}/>)}
      </div>
    </div>
  )
}
